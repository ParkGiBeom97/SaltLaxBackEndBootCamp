...
public class BookService {
	...
	public ObservableList<BookVO> deleteByISBN(String deleteISBN, String searchKeyword) {
		...
		// 여기서부터 트렌젝션 시작이에요
		//connection에 대해서 setAutocommit()을 false로 지정해야 Transaction이 시작함
		
		
		//여기서 데이터베이스 커넥션을 얻어온 후 DAO에게 주입함
		Connection con = null;
		try {
			con = (DBCPConnectionPool.getDataSource()).getConnection();
			con.setAutoCommit(false);
		...
		//여기 부분이 트랜잭션 끝
		//서비스의 메소드 맨 마지막에 이 메소드가 잘 처리 됐으면
		//이 로직 코드는 우리가 처리해야함
		// transaction을 commit하고 그렇지 않으면
		//rollback시킴
