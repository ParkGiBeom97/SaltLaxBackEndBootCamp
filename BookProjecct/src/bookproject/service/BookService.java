...
public class BookService {
	...
	public static Boolean loginUser(String text, String text2) {
		...
		try {
			...
			if (text.equals(dao.userLogin(text, text2).getUser_ID())
					&& text2.equals(dao.userLogin(text, text2).getUser_PW())) {
				loginResult = true;
			}
	...
	public ObservableList<BookVO> addBookService(String text, String text2, String text3, int parseInt, int parseInt2,
		...
		try {
			...
			int count = dao.addBook(text, text2, text3, parseInt, parseInt2, text4, text5, text6, text7, parseInt3);
	...
	public ObservableList<BookVO> updateBookService(String text, String text2, String text3, int parseInt,
		...
		try {
			...
			int count = dao.updateBook(text, text2, text3, parseInt, parseInt2, text4, text5, text6, text7, parseInt3,
					bookISBN);
	...
	public LogVO getPointService(String bisbn, String idID) {
		...
		try {
			...
			long sec = (dao.getBDate(bisbn).getReturnDay().getTime() - dao.getBDate(bisbn).getRetalDay().getTime())
					/ 1000;
	...
	public ObservableList<BookVO> addBookServiceMB(String text, String text2, String text3, int parseInt, int parseInt2,
		...
		BookDAO dao = new BookDAO(factory);
		ObservableList<BookVO> listMB = dao.insertBookMB(text, text2, text3, parseInt, parseInt2, text4, text5, text6,
				text7, parseInt3);
		return listMB;
	}

	// 마이바티스로 책 수정하는 서비스
	public ObservableList<BookVO> updateBookServiceMB(String text, String text2, String text3, int parseInt,
			int parseInt2, String text4, String text5, String text6, String text7, int parseInt3, String bookISBN) {
		BookDAO dao = new BookDAO(factory);

		ObservableList<BookVO> listMB = dao.updateBookMB(text, text2, text3, parseInt, parseInt2, text4, text5, text6,
				text7, parseInt3, bookISBN);

		return listMB;
	}

	// 마이바티스로 책 삭제하는 서비스
	public ObservableList<BookVO> deletBServiceMB(String bookISBN, String searchKeyword) {
		BookDAO dao = new BookDAO(factory);

		int delete = dao.deleteBBookMB(bookISBN, searchKeyword);

		ObservableList<BookVO> listMB = dao.selectAllBook(searchKeyword);

		return listMB;
	}

	// 마이바티스로 대여 테이블에서도 책 삭제하는 서비스
	public RentalVO deletRServiceMB(String bookISBN) {
		BookDAO dao = new BookDAO(factory);

		int delete = dao.deleteRBookMB(bookISBN);

		return null;
	}

	// 마이바티스로 유저정보 수정하는 서비스
	public UserVO setUserDateServiceMB(String text, String text2, String text3, String idID) {
		BookDAO dao = new BookDAO(factory);

		int userUpdate = dao.updateUserMB(text, text2, text3, idID);

		return null;
	}

	// 마이바티스로 책정보 가져오기
	public BookVO getBIServiceMB(String bookISBN) {
		BookDAO dao = new BookDAO(factory);

		BookVO book = dao.getBookInfoMB(bookISBN);

		return book;
	}
	
	//마이바티스로 전체 대여 현황 가져오기
	public ObservableList<RentalVO> RentBookSMB() {
		BookDAO dao = new BookDAO(factory);
		
		ObservableList<RentalVO> listMB = dao.getRBookDAO();
		
		return listMB;
	}
	
	//마이바티스로 회원 정보 가져오기
	public UserVO getInfoServiceMB(String idID) {
		BookDAO dao = new BookDAO(factory);
		
		UserVO user = dao.getUserInfoMBE(idID);
