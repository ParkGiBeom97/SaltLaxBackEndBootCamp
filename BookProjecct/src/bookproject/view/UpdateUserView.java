...
public class UpdateUserView {
	...
	public BorderPane setUser(String idID) {
		...
		//마이바티스로 원래 회원 정보 받아오기
		UserVO userInfoMB = controllerUser.getInfoControllerMB(idID);
		...
		upDateUser.setOnAction(e -> {
			...
			/*
			 * UserVO user = controller.upDateUserController(user_ID_tf.getText(),
			 * user_PW_tf.getText(), user_EM_tf.getText(), idID);
			 */

			// 마이바티스로 유저 정보 수정하기
			
			UserVO user = controller.upDateUserControllerMB(user_ID_tf.getText(), user_PW_tf.getText(),
					user_EM_tf.getText(), idID);
